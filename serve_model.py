...
def evaluate_batch_ctc(args, model, batch, valid_len, inf, vocab):
    ...
    if args.bpe == True:
        transcript = apply_lex(args.sp.decode(best_combined[len(best_combined)-1][0]).lower(), vocab)
    else:
        transcript = apply_lex(re.sub(r"[#^$]+", "", best_combined[len(best_combined)-1][0].lower()), vocab)
    return transcript
